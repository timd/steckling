...
- Quitting **stops the branch's services too** (data kept — same as `steck down`): the cockpit
  is the branch session, opened and closed as one thing. Pass `--keep-up` to quit the viewport
