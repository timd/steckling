...
import { metaVars, portEnvName, portPlaceholders, readDotenv, resolveEnv, stecklingDir, writeDotenv } from "./env";
import {
  currentBranch,
  deleteBranch,
  isMerged,
...
import { runInherit, which } from "./sh";
...
/**
 * `steck cockpit` — a per-branch cockpit TUI, delegated to mprocs (the same way
 ...
 */
export async function cockpit(opts: { keepUp: boolean }): Promise<number> {
  if (!which("mprocs")) {
    log.error("`steck cockpit` needs mprocs (the cockpit TUI), which wasn't found on PATH.");
    log.info("Install it with: brew install mprocs   (or: cargo install mprocs)");
    return 1;
  }

  // Bring the stack up exactly as `steck up --no-run` would (ports, compose
  // up --wait, .steckling/env, registry, provision-once).
  const upCode = await up({ noRun: true, reprovision: false });
  ...
  // The app pane gets exactly what `steck up` would hand the app.
  const appEnv = readDotenv(ctx.worktreeDir) ?? {};

  // Log panes interpolate the compose file, so they need the port vars —
  // real ports where recorded, placeholders for unexposed services.
  const services = composeFileServiceNames(ctx.composeFile);
  const record = loadRegistry().worktrees[ctx.names.project];
  const composeEnv = portPlaceholders(services);
  for (const [name, port] of Object.entries(record?.ports.services ?? {})) {
  ...
  const configPath = join(stecklingDir(ctx.worktreeDir), "mprocs.yaml");
  writeFileSync(
    configPath,
    `# Generated by \`steck cockpit\` for branch ${ctx.branch} — rewritten on every run.\n` +
      stringifyYaml({ procs }),
  );
  ...
  if (stopped.stopped > 0) {
    log.ok(
      `Cockpit closed — stopped ${stopped.stopped} container(s), data kept. \`steck up\` or \`steck cockpit\` to resume.`,
    );
  } else {
